...
if @isdefined time_to_download
	md"""The starting input file is $(filesize(filename_csv)) bytes.
...
begin
	...
	for (i, n) in enumerate(sort(n_rows_for_benchmarks, rev=true))
...
if !(@isdefined already_benchmarked_csv_read)
	time_to_read_csv_small_first # force to wait until after benchmarking small files 
...
md"""
Note that in the cell above, we added some optional arguements to `CSV.read` to tell it what variable type to use for one column and to tell it which string use to interpret as meaning there's missing data.  There are many optional arguements to provide finer control like this.  See the function [documentation](https://csv.juliadata.org/stable/reading.html#CSV.read) for details.  
"""
...
begin
	...
	@elapsed save(filename_jld2_small, Dict("small_df" => small_df) ) # Force compilation
...
begin
	...
	@elapsed write_dataframe_as_fits(filename_fits_small,small_df) # force compilation
	# Write data to FITS file
	time_to_write_fits_small = @elapsed write_dataframe_as_fits(filename_fits_small,small_df)
	# Read data back from FITS file
	time_to_read_fits_small_first = @elapsed df_fits = read_fits_tables(filename_fits_small)
	time_to_read_fits_small_second = @elapsed df_fits = read_fits_tables(filename_fits_small)
end;
...
protip(md"""
I'm not entirely sure why the runtime for reading/writing the real-world FITS files is significantly greater than for HDF5/JLD2 files.   I'm guessing it's that HDF5 and JLD2 are better optimized for working with strings.
